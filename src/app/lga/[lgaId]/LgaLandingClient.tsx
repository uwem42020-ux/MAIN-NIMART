...
interface LgaLandingClientProps {
  lgaName: string;
  stateName: string;
  providers: ProviderWithProfile[];
  categoriesWithCount: { slug: string; name: string; count: number }[];
}

export default function LgaLandingClient({
  lgaName,
  stateName,
  providers,
  categoriesWithCount,
}: LgaLandingClientProps) {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="bg-gradient-to-r from-primary-700 to-green-600 text-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 md:py-16">
          <div className="flex items-center gap-2 text-sm text-primary-100 mb-4">
            ...
            <Link href={`/providers/${stateName.toLowerCase().replace(/\s+/g, '-')}`} className="hover:text-white">{stateName}</Link>
            <ChevronRight className="h-4 w-4" />
            <span className="text-white">{lgaName}</span>
          </div>
          <h1 className="text-3xl md:text-4xl font-bold mb-3">
            Trusted Service Providers in {lgaName}, {stateName}
          </h1>
          <p className="text-primary-100 max-w-2xl text-base md:text-lg">
            Browse {totalProviders} verified professionals in {lgaName}. Compare ratings, read reviews,
            and book trusted local services on Nimart.
          </p>
          ...
          <div className="grid grid-cols-3 gap-4 mt-8 max-w-md">
            ...
            <div className="bg-white/10 backdrop-blur-sm rounded-xl p-4 text-center">
              ...
              <p className="text-2xl font-bold">{categoriesWithCount.length}</p>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        {/* Categories in this LGA */}
        {categoriesWithCount.length > 0 && (
          <section className="mb-10">
            <h2 className="text-xl font-bold text-gray-900 mb-4">
              Popular Categories in {lgaName}
            </h2>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              {categoriesWithCount.map((cat) => (
                <Link
                  key={cat.slug}
                  href={`/categories/${cat.slug}/${stateName.toLowerCase().replace(/\s+/g, '-')}`}
                ...
                >
                  ...
                  <p className="text-xs text-primary-600 mt-1">{cat.count} provider{cat.count !== 1 ? 's' : ''}</p>
                </Link>
              ))}
            </div>
          </section>
        ...
        <section>
          <div className="flex items-center justify-between mb-5">
            ...
            <div className="flex gap-2">
              ...
              <Link
                href={`/search?lga=${encodeURIComponent(lgaName)}`}
        ...
        <section className="mt-12 bg-white rounded-2xl shadow-sm border border-gray-100 p-6 md:p-8">
          ...
          <div className="space-y-5">
            <div>
              <h3 className="font-semibold text-gray-900">How do I find trusted providers in {lgaName}?</h3>
              <p className="text-gray-600 mt-1 text-sm">
                Browse the provider list above, check ratings and reviews, and book directly through Nimart.
              </p>
            </div>
            <div>
              <h3 className="font-semibold text-gray-900">Are the providers in {lgaName} verified?</h3>
              <p className="text-gray-600 mt-1 text-sm">
